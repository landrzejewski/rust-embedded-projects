...
use esp_idf_svc::{hal::{delay::FreeRtos, gpio::{IOPin, InterruptType, PinDriver, Pull}, prelude::Peripherals}, log::EspLogger, sys::link_patches};

const START_DELAY_IN_MILLS: u32 = 100;
...
fn main() {
    ...
    let mut delay = START_DELAY_IN_MILLS;

    loop {
        for led_pin in &mut led_pins {
            if FLAG.load(Ordering::Relaxed) {
                delay = delay + DELAY_CHANGE_IN_MILLIS;
            ...
            FreeRtos::delay_ms(START_DELAY_IN_MILLS);
        }
    }
}
